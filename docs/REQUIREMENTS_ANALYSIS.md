...
- Выполнять полный цикл: валидация → rate limit → запись со статусом
  `processing` → ответ `202` → фоновая AI-классификация → два письма →
  сохранение итоговых статусов.
- После успешного сохранения возвращать `202 Accepted`; ответ содержит только
  `request_id`, `status` и `message`.
- Сбой AI не отменяет сохранение и стандартные письма. AI и письма выполняются
  через `BackgroundTasks`; SMTP-сбои сохраняются в БД и логируются.
...
4. **Синхронный полный цикл и `BackgroundTasks`:** поток из девяти шагов можно
   ...
   сохранения. AI и письма выполняются одной фоновой задачей с отдельной
   DB-сессией; итоговые статусы сохраняются в БД.
...
11. **AI-категория в ответе:** начальный ответ `202` не содержит AI-результат,
    потому что классификация выполняется в фоне. Результат сохраняется в БД и
    доступен в защищённом `GET /api/contacts`.
